...
public class ImageCycleView extends LinearLayout {
	...
	public ImageCycleView(Context context, AttributeSet attrs) {
		...
		LayoutInflater.from(context).inflate(R.layout.layout_image_cycle, this);
		mBannerPager = (CycleViewPager) findViewById(R.id.id_image_cycle_viewpager);
		...
		// 滚动图片右下指示器视图
		mGroup = (ViewGroup) findViewById(R.id.id_image_cycle_viewGroup);
	}


	public void setImageResources(ArrayList<ImageItem> infoList, ImageCycleViewListener imageCycleViewListener) {
		// 清除所有子视图
		...
		for (int i = 0; i < imageCount; i++) {
			...
			if (i == 0) {
				mImageViews[i].setBackgroundResource(R.drawable.ic_dot_focused);
			} else {
				mImageViews[i].setBackgroundResource(R.drawable.ic_dot_normal);
			}
	...
	private class ImageCycleAdapter extends PagerAdapter {
		 ...
		 * 图片资源列表
		 */
		private ArrayList<ImageItem> mAdList = new ArrayList<ImageItem>();
		...
		public ImageCycleAdapter(Context context, ArrayList<ImageItem> adList, ImageCycleViewListener imageCycleViewListener) {
			mContext = context;
			mAdList = adList;
